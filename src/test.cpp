...
void test_gauss_with_velocity_with_barrier_step() {
    ...
    potentialParams.barrier_V0 = 20.0;
...
void test_klein_gordon_1D() {
    gridParams.m = 0.0;
    gridParams.dt = 0.4 * gridParams.dx * gridParams.dx;
    gridParams.T = 5.0;
    gridParams.Nt = int(gridParams.T / gridParams.dt);
    gaussParams.k = 0.0;
    potentialParams.type = PotentialType::None;

    std::cout << "Running Klein-Gordon solver in 1D with Gaussian initial condition without potential...\n" << std::endl;
    std::cout << "dt: " << gridParams.dt << ", Nt: " << gridParams.Nt << std::endl;
    std::cout << "T real: " << gridParams.Nt * gridParams.dt << std::endl;
    klein_gordon_1D();
}